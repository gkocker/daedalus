"""
api.model — declarative model input (the primary authoring path).
...
``TemporalModelBuilder`` describes time-only (SDE) models;
``SpatialModelBuilder`` is the spatial (SPDE) analogue.  Both subclass
...
Typical use::
    ...
    model = (TemporalModelBuilder("My model")
...
Reusable actions live in ``api.model_templates`` (e.g. ``HawkesAction``)
and are applied via ``.use_action_template(...)`` instead of
``.set_action_text(...)``.  For unusual models that no string or
...
class _SpatialMethods:
    """Spatial-only authoring methods (mixed into SpatialModelBuilder
    and the back-compat ModelBuilder)."""
    ...
    def boundary(self, mode: str, **params):
        ...
        mode : {'infinite', 'periodic'}
            ...
            ``.boundary`` is never called on a spatial model).
...
class _TemporalMethods:
    """Temporal-only authoring methods (mixed into TemporalModelBuilder
    and the back-compat ModelBuilder)."""
    ...
    def population(self, name: str, *, size: int = 1,
        ...
        Heterogeneous-population models chain one ``.population()``
...
class _BaseModelBuilder:
    ...
    Shared base for ``TemporalModelBuilder`` and ``SpatialModelBuilder``.
    ...
    hatch for models no string or template covers.
    """
    ...
    def __init__(self, name: str, n_populations: int = 1):
        ...
        # via ``api.model_compiler``, overriding the lambda hooks
        ...
        # the stable subset.  Default OFF — models that integrate
        # out their voltages (all-algebraic equations, no ``Dt``)
        # have no differential structure to score, so eigenvalue
        # analysis is vacuous.  Bistable / differential models that
        ...
        # ``.markovianize(False)`` for models whose colored kernels
        ...
        # models.  See docs/dyson_duhamel_integration_plan.md (D-4).
    ...
    def physical_field(self, name: str, indexed: bool = True,
        ...
        Every spatial field in a model must share the same non-zero
        ...
        **Legacy style** (existing hand-written model files)::
    ...
    def operator_ir(self, on: bool = True):
        ...
        factors are represented exactly.  Default OFF — every existing model is
    ...
    def equation(self, *, lhs: str, rhs: str,
        ...
        if not isinstance(lhs, str) or not lhs.strip():
            raise ValueError(
                'ModelBuilder.equation(): lhs must be a non-empty string.'
            )
        if not isinstance(rhs, str) or not rhs.strip():
            raise ValueError(
                'ModelBuilder.equation(): rhs must be a non-empty string.'
            )
        ...
        for side_name, side_text in (('lhs', lhs), ('rhs', rhs)):
            if re.search(r'\bConv\s*\(', side_text, re.IGNORECASE):
                raise ValueError(
                    f"ModelBuilder.equation(): Conv(...) found in "
        ...
        if re.search(r'\bDt\b', rhs):
            raise ValueError(
                "ModelBuilder.equation(): Dt found in rhs; time "
    ...
    def stability_analysis(self, enabled: bool):
        ...
        enabled : bool
            ...
            bistable / multi-saddle models where the diagrammatic
            ...
            first index.  Use this for models whose equations are
    ...
    def _auto_populate_mf_eqs_from_equations(self) -> None:
        ...
        Models with more elaborate LHSs (e.g. ``v[i] - n[i] = …``)
        ...
        for eq in self._equations:
            ...
            if primary is None:
                raise ValueError(
                    ...
                    f'models) or an indexed one (``v[i]``).  Either '
    ...
    def _compile_text_declarations(self) -> None:
        ...
        from api.model_compiler import (
            make_action_lambda,
    ...
    def _inject_autopop(self) -> None:
        ...
        Only fires for scalar models (``n_populations <= 1``); the
        ``build()`` caller gates on that.  Legacy multi-population models
    ...
    def _resolve_spatial_dim(self) -> int:
        """Resolve the model's single spatial dimension from the physical
        ...
        if len(nonzero_dims) > 1:
            raise ValueError(
                f'ModelBuilder("{self.name}").build(): v1 does not '
    ...
    def build(self) -> dict:
        ...
        # wrappers in ``model_compiler``, this lets users write
        # truly scalar models — bare ``xt * x`` instead of
        # ``sum(xt[i]*x[i] for i in pop)``.
        #
        # Gate on ``n_populations <= 1``: the autopop always injects a
        # size-1 population, so firing it for a legacy
        # ``n_populations >= 2`` model (e.g. the 2-pop Hawkes models,
        ...
        # every cross-population correlator.  Those models instead
        ...
        if missing:
            raise ValueError(
                f'ModelBuilder("{self.name}").build(): missing required '
        ...
        # Compute the model's spatial dimension, validate the v1
        ...
        # ``docs/spatial_design_decisions_v1.md``.
        model_spatial_dim = self._resolve_spatial_dim()
        is_spatial = model_spatial_dim > 0
        ...
        # they're reserved in EVERY model.  ``k`` (wavevector,
        ...
        # model is spatial.  Time-only models keep ``x`` free — it's
        ...
        for kind, nm in _named:
            if nm and nm in reserved:
                ...
                raise ValueError(
                    ...
                    f'In a spatial model, x/y/z are the spatial '
        ...
        # the model is spatial so ``field_theory._build_namespace``
        ...
        if self._boundary is not None:
            if not is_spatial:
                raise ValueError(
                    f'ModelBuilder("{self.name}").build(): .boundary() '
        ...
        model = {
            ...
            # None for template-built models; surfaced by
            ...
            # for bistable / multi-saddle differential models.
        ...
        if is_spatial:
            ...
            # Boundary defaults to 'infinite' when the model is spatial
            ...
            # on a non-spatial model already raised in the resolution
...
class TemporalModelBuilder(_TemporalMethods, _BaseModelBuilder):
    """Authoring API for time-domain (non-spatial) models: carries the
    ...
    def _resolve_spatial_dim(self) -> int:
        ...
        if d > 0:
            raise ValueError(
                f'TemporalModelBuilder("{self.name}"): a physical field '
                f'declared spatial_dim={d} > 0.  Use SpatialModelBuilder for '
                f'spatial (PDE) models.')
        return 0


class SpatialModelBuilder(_SpatialMethods, _BaseModelBuilder):
    """Authoring API for spatial (PDE / field-theory) models: carries the
    spatial-only methods (spatial_dim, boundary, initial); has no temporal
    kernel/colored-noise methods.  Multi-field spatial models use repeated
    ...
    def _resolve_spatial_dim(self) -> int:
        ...
        if d <= 0:
            raise ValueError(
                f'SpatialModelBuilder("{self.name}"): no physical field is '
                f'spatial.  Set spatial_dim>=1 on a field '
                f'(physical_field(..., spatial_dim=d)) or call .spatial_dim(d).')
        return d


class ModelBuilder(_SpatialMethods, _TemporalMethods, _BaseModelBuilder):
    ...
    Behaviorally identical to the pre-split ModelBuilder.  New code should
    prefer TemporalModelBuilder / SpatialModelBuilder."""
    pass


# ── deprecated aliases (pre-July-2026 name: "theory") ────────────────────────
TheoryBuilder = ModelBuilder
TemporalTheoryBuilder = TemporalModelBuilder
SpatialTheoryBuilder = SpatialModelBuilder
